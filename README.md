...
```
conda(mamba) create -n basil_env -c samrachhan11 -c conda-forge -c bioconda -c defaults basil=1.1 -y
...
```
sample_id_filt.fasta: This is the final assembly that you should use.
sample_id_polished.fasta: This is the polished assembly before filtering.
```
## --meta_merge
...
# Report
```Main_report.html``` is a report generated by MultiQC that consists of the outputs from Read QC, Assembly QC, and Speciation.
```read_qc_summary_report.html``` is a report that provides several key read QC metrics calculated internally—including total reads, total bases, mean read length, genome size, estimated genome coverage, and downsampling information—along with the final QC status and its associated reasoning.
