...
def generate_html(consensus: dict, output_path: str, minid_genes: float, mincov_genes: float) -> None:
    ...
    db_legend_chips = "".join(
        f'<span class="db-chip" '
        f'style="background:{m["color"]}22;border-color:{m["color"]}55;color:{m["color"]}">'
        f'{m["sup"]} {m["name"]}</span>'
        for m in DB_META.values()
    )

    db_stat_cards = "".join(
        f'<div class="stat-card db-stat">'
...
<div class="stats-section">
  ...
  <!-- Per-database gene counts (static from Python) -->
  <div class="stats-bar" id="dbStatsBar">
    {db_stat_cards}
  </div>
  <!-- Database legend -->
  <div class="legend-bar">
    <span class="legend-title">Database Key &nbsp;&nbsp;</span>
    {db_legend_chips}
  </div>
</div>
