...
const TAB_COLS = {{
  assembly: [
    ...
    'asm1_n50','asm1_length','asm1_contigs',
    'asm2_n50','asm2_length','asm2_contigs',
    'best_assembly', null
  ],
  filtering: [
...
function renderAssembly(data) {{
  ...
  tbody.innerHTML = data.map(r => {{
    ...
    return `<tr>
      ...
      <td>${{fmtN(r.asm1_n50)}} bp</td><td>${{fmtN(r.asm1_length)}} bp</td><td>${{fmtN(r.asm1_contigs)}}</td>
      <td>${{fmtN(r.asm2_n50)}} bp</td><td>${{fmtN(r.asm2_length)}} bp</td><td>${{fmtN(r.asm2_contigs)}}</td>
      <td>${{bestBadge}}</td>
...
const HEADS = {{
  assembly:  ['Sample ID','Coverage (X)','Mode','K-mers (spades)',
               'Asm1 N50 (bp)','Asm1 Length (bp)','Asm1 Contigs',
               'Asm2 N50 (bp)','Asm2 Length (bp)','Asm2 Contigs',
...
function getRows(tab) {{
  return state[tab].filtered.map(r => {{
    if (tab==='assembly') return [
      ...
      r.asm1_n50??'',r.asm1_length??'',r.asm1_contigs??'',
      r.asm2_n50??'',r.asm2_length??'',r.asm2_contigs??'',
...
function exportCSV() {{
  ...
  downloadBlob(rows.map(r=>r.map(v=>`"${{xlEsc(String(v)).replace(/"/g,'""')}}"`).join(',')).join('\\n'),
    `assembly_${{activeTab}}_report.csv`,'text/csv');
}}
function exportTSV() {{
  const rows=[HEADS[activeTab],...getRows(activeTab)];
  downloadBlob(rows.map(r=>r.map(v=>xlEsc(String(v))).join('\\t')).join('\\n'),
