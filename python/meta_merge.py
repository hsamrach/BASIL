...
SUPPORTED_EXTENSIONS: frozenset = frozenset({'.xlsx', '.xls', '.csv', '.tsv', '.tab', '.txt'})

_ENCODINGS = ('utf-8-sig', 'cp1252', 'latin-1')

def _read_csv_tsv(filepath: str, sep: str) -> pd.DataFrame:
    """Try encodings in order. latin-1 maps all 256 bytes so it always succeeds."""
    last_exc: Exception | None = None
    for enc in _ENCODINGS:
        try:
            return pd.read_csv(filepath, sep=sep, header=0, dtype=str, encoding=enc)
        except UnicodeDecodeError as exc:
            last_exc = exc
    raise last_exc  # unreachable if latin-1 is last, satisfies type-checker
...
def load_file(filepath: str) -> pd.DataFrame:
    ...
    try:
        if ext in ('.xlsx', '.xls'):
            df = pd.read_excel(filepath, header=0, dtype=str)   # openpyxl handles encoding
        elif ext == '.csv':
            df = _read_csv_tsv(filepath, sep=',')
        else:  # .tsv / .tab
            df = _read_csv_tsv(filepath, sep='\t')
