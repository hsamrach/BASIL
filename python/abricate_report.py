...
def main(input_dir, output_html, minid_genes, mincov_genes):
...
<header>
  <div>
    ...
    <div style="font-size:0.78rem;color:var(--muted);margin-top:4px;font-family:var(--font-main);">Minimum Identity: {minid_genes}% / Minimum Coverage: {mincov_genes}%</div>
  </div>
...
if __name__ == "__main__":
    ...
    minid_genes = float(sys.argv[3]) if len(sys.argv) > 3 else 90.0
    mincov_genes = float(sys.argv[4]) if len(sys.argv) > 4 else 90.0
    main(input_dir, output_html, minid_genes, mincov_genes)
