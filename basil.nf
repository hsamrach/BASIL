...
static final Set VALID_PARAMS = [
    ...
    'kmer_min_count', 'kmer_size','help', 'version', 'clear_saved_db', 'constants', 'meta_merge'
] as Set
...
def collected_contig_ch = format_convert_contigs.out.collected_dir
    ...
    if (needs_collection) {
        all_done = all_done
            .mix(gene_predict_abricate.out[0])  // TSV reports
            .mix(gene_predict_abricate.out[1])  // HTML report
            .mix(gene_predict_abritamr.out[0]) // partial matches report
            .mix(gene_predict_abritamr.out[1]) // virulence report
            .mix(gene_predict_abritamr.out[2]) // matches report
            .mix(gene_predict_abritamr.out[3]) // HTML report
            .mix(gene_predict_amr_consensus.out.html_report)  // consensus HTML
            .mix(gene_predict_amr_consensus.out.tsv_summary)  // consensus TSV
    }
