...
process speciation_kraken2_contigs_dir {

    label 'kraken2_contigs_dir'
    ...
    output:
    path "kraken2_contigs_reports", emit: kraken2_contigs_reports

    script:
    """

    mkdir -p kraken2_contigs_reports
    ...
    for fasta in ${collected_dir}/*.fasta; do
        ...
        kraken2 \\
            --db ${kraken_db} \\
            --report kraken2_contigs_reports/\${sample}.report \\
            --output kraken2_contigs_reports/\${sample}.txt \\
            --threads ${params.cpus} \\
            --use-names \\
            \$fasta
    done
